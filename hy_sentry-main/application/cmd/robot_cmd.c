...
static Subscriber_t *GimbalBase_sub; // 底盘反馈信息订阅者
static Subscriber_t *c;   // 母云台电机数据
#endif                                 // ONE_BOARD
...
void RobotCMDInit()
{
    ...
    gimbal_cmd_pub = PubRegister("gimbsal_cmd", sizeof(Gimbal_Ctrl_Cmd_s));
...
static void RadarControlSet()
{
    ...
    if(switch_is_up(rc_data[TEMP].rc.switch_left))
    {
        ...
        chassis_cmd_send.vx = (float)radar_data->linear.x * 150;
        chassis_cmd_send.vy = -(float)radar_data->linear.y * 150;
        chassis_cmd_send.wz = (float)radar_data->angular.z * 0;
        }
    }
    else if(switch_is_mid(rc_data[TEMP].rc.switch_left)) // 左侧开关状态为[中],遥控
    {
    ...
    else if(switch_is_down(rc_data[TEMP].rc.switch_left)) // 左侧开关状态为[下],停止
    {
        chassis_cmd_send.vx = 0.0f; // 竖直方向
        chassis_cmd_send.vy = 0.0f; // 水平方向
        chassis_cmd_send.wz = 0.0f;    // 旋转速度
    }
}

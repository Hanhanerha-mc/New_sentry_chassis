...
#define OUTPUT_LPF_RC 0.1f
    ...
    motor_rf = DJIMotorInit(&chassis_first_M3508_motor_config);
...
static void LimitChassisOutput()
{
    ...
    DJIMotorSetRef(motor_rf, chassis_handle.motor_set_speed[0]);
    DJIMotorSetRef(motor_lf, chassis_handle.motor_set_speed[1]);
    DJIMotorSetRef(motor_lb, chassis_handle.motor_set_speed[2]);
    DJIMotorSetRef(motor_rb, chassis_handle.motor_set_speed[3]);
    // DMMotorSetRef(Gimbal_Base, chassis_handle.gimbal_angle);
    // Gimbal_Base->pid_ref=chassis_handle.gimbal_angle;
    // DMMotorSetRef(Gimbal_Base, 100.0f);
}
...
static void Steer_Speed_Calcu(ChassisHandle_t *chassis_handle, float chassis_vx, float chassis_vy, float chassis_wz)
{
    chassis_handle->chassis_motor_speed[0] = ((-chassis_vx+chassis_vy)*COSINE45 - chassis_wz * RF_CENTER*0.2)*50; 
    chassis_handle->chassis_motor_speed[1] = ((-chassis_vx-chassis_vy)*COSINE45 - chassis_wz * LF_CENTER*0.2)*50; 
    chassis_handle->chassis_motor_speed[2] = ((chassis_vx-chassis_vy)*COSINE45 - chassis_wz * LB_CENTER*0.2)*50; 
    chassis_handle->chassis_motor_speed[3] = ((chassis_vx+chassis_vy)*COSINE45 - chassis_wz * RB_CENTER*0.2)*50;
...
static void Steer_Calculate(ChassisHandle_t *chassis_handle, float chassis_vx, float chassis_vy, float chassis_wz)
{
    //Steer_angle_change(chassis_handle, chassis_vx, chassis_vy, chassis_wz); //原来的舵轮，现在屏蔽了
...
{   
    ...
    // chassis_handle.vy = chassis_cmd_recv.vy * cos_theta + chassis_cmd_recv.vx * sin_theta;
    // chassis_handle.vx = chassis_cmd_recv.vx * cos_theta - chassis_cmd_recv.vy * sin_theta;
    chassis_handle.vy = chassis_cmd_recv.vy;
    chassis_handle.vx = chassis_cmd_recv.vx;
    chassis_handle.wz = chassis_cmd_recv.wz;
    // if(referee_data->PowerHeatData.buffer_energy<200){
    //     chassis_handle.vy = chassis_cmd_recv.vy*(referee_data->PowerHeatData.buffer_energy/200.0f);
    //     chassis_handle.vx = chassis_cmd_recv.vx*(referee_data->PowerHeatData.buffer_energy/200.0f);
    //     chassis_handle.wz = chassis_cmd_recv.wz*(referee_data->PowerHeatData.buffer_energy/200.0f);
    // }
    // if(referee_data->PowerHeatData.buffer_energy<30){
    // chassis_handle.vy = chassis_cmd_recv.vy * 0.5;
    // chassis_handle.vx = chassis_cmd_recv.vx * 0.5;
    // chassis_handle.wz = chassis_cmd_recv.wz * 0.5;
    // }


    // chassis_handle.gimbal_angle = chassis_cmd_recv.gimbal_angle*0; 

    // chassis_handle.gimbal_angle = chassis_cmd_recv.gimbal_angle+chassis_cmd_recv.offset_angle;
    // ChassisHandle_Deliver_Config();

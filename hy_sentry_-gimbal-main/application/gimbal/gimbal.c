...
// 电机软件限位
#define YAW_L_LIMIT_MIN -90
#define YAW_L_LIMIT_MAX 40

#define PITCH_L_LIMIT_MIN 95
#define PITCH_L_LIMIT_MAX 128
...
{   
    ...
    Motor_Init_Config_s dm_yaw_config = {
        .can_init_config = {
            .can_handle = &hcan1,
            .tx_id = 0x20f,  // 达妙电机的can id,需要根据实际情况修改,目前是速度模式
            .rx_id = 0x210,  // 达妙电机的master id,需要根据实际情况修改,目前是速度模式
        },
    ...
    Motor_Init_Config_s pitch_config = {
        ...
        .controller_param_init_config = {
            .angle_PID = {
                ...
                .Ki = 1, // 3  80
                .Kd = 0.3,
                .Improve = PID_Trapezoid_Intergral | PID_Integral_Limit | PID_ADAPTIVE_Proportional,
                .IntegralLimit = 0.3, // 30
                .MaxOut = 1000,

                .Pmin = 80, // 线性自适应的p最小值
                .Mmin = PITCH_L_SEND_MIN, // 线性自适应的误差最小值
                .Mmax = PITCH_L_SEND_MAX, // 线性自适应的误差最大值
            },
            .speed_PID = {
                .Kp = 35,  // 50    空载k = 10  Me: 46
                .Ki = 200, // 0
                .Kd = 0,   // 0
                .Improve = PID_Trapezoid_Intergral | PID_Integral_Limit | PID_Derivative_On_Measurement | PID_OutputFilter,
                .Output_LPF_RC=0.00649999983,
    ...
    */
    ...
    dm_yaw = DMMotorInit(&dm_yaw_config);        //最后再进行初始化，确保dm电机已经完全上电
}
    ...
    if(vision_recv_data_r.target_state == NO_TARGET)
    {
       // Gimbal_T += GimbalDirection;
        ...
        if(diff_time > 500)     // 超时后开始扫描
        {
            ...
            time_tick_ +=0.005f;
            vision_l_yaw_tar = ((YAW_L_LIMIT_MIN + YAW_L_LIMIT_MAX) / 2) + (((YAW_L_LIMIT_MAX - YAW_L_LIMIT_MIN) / 2) * arm_sin_f32(time_tick_ * 1.60f));
            // //  //vision_l_yaw_tar = YAW_L_LIMIT_MIN + (YAW_L_LIMIT_MAX -  YAW_L_LIMIT_MIN) / 2;
            // vision_l_pitch_tar = ((PITCH_L_LIMIT_MIN + PITCH_L_LIMIT_MAX) / 2) + (((PITCH_L_LIMIT_MAX - PITCH_L_LIMIT_MIN) / 2) * arm_sin_f32(time_T * 1.6));
        ...
        if(raw_pitch_input>-0.05f && raw_pitch_input<0.05f)
        {
...
{   
    ...
    case GIMBAL_FREE_MODE: // 后续删除,或加入云台追地盘的跟随模式(响应速度更快)
        ...
        DJIMotorSetRef(pitch_l_motor, gimbal_cmd_recv.pitch);       // 适配遥控器，没加初始角度

        DMMotorEnable(dm_yaw);
        DMMotorChangeFeed(dm_yaw, ANGLE_LOOP, OTHER_FEED);
        DMMotorSetRef(dm_yaw, gimbal_cmd_recv.dm_yaw);

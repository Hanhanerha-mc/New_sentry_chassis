...
typedef enum
{
    ...
    PID_ADAPTIVE_Proportional = 0b00001000,           // 0000 1000 自适应比例
...
typedef struct
{
    ...
    float Pmin;              // 线性自适应的p最小值
    float Mmin;              // 线性自适应的误差最小值,当误差小于此值时,使用Pmin计算Pout
    float Mmax;              // 线性自适应的误差最大值,当误差大于此值时,使用Kp计算Pout
    //-----------------------------------
    // for calculating
    float Measure;           // 当前测量值
